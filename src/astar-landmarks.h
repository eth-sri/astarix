...
class AStarLandmarks: public AStarHeuristic {
  ...
  public:
    ...
    cost_t h(const state_t &st) const {
        return (pivots - H[st.v]) * costs.get_min_mismatch_cost();
    }
  ...
  private:
    ...
    bool update_path_backwards(int i, node_t v, int dval) {
        ...
        for (auto it=G.begin_orig_rev_edges(v); it!=G.end_orig_rev_edges(); ++it) {
            ...
            if (should_proceed_backwards_to(i-1, it->to)) {
                assert(update_path_backwards(i-1, it->to, dval));
    ...
    void match_pivot_and_update(const read_t *r, int start, int pivot_len, int i, node_t v, int dval) {
...
//                match_pivot_and_update(r, start, pivot_len, i+1, it->to, dval);
            ...
            assert(update_path_backwards(i, v, dval));
    ...
    int gen_pivots_and_update(const read_t *r, int pivot_len, int dval) {
        ...
        for (int i=r->len-pivot_len+1; i>0; i-=pivot_len) {
            ...
            match_pivot_and_update(r, i, pivot_len, i, 0, dval);
            pivots++;
        }
        return pivots;
    }
