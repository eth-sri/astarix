...
///////////////////////////////////////
// in case max_seed_errors == 0
///////////////////////////////////////

class AStarSeedsExact: public AStarHeuristic {
  private:
    // Fixed parameters
    const graph_t &G;
...
//    std::vector<int> visited;  // == +1 if a node has already been added to H; -1 if a node has already been subtracted from H
  ...
  public:
    ...
    cost_t h(const state_t &st) const {
        int max_seeds_to_end = (r_->len - st.i - 1) / pivot_len;
        int h_remaining = H[st.v] & ((1<<max_seeds_to_end)-1);
        int matching_seeds = __builtin_popcount(h_remaining);
        assert(max_seeds_to_end >= matching_seeds);
        cost_t res = (max_seeds_to_end - matching_seeds) * costs.get_min_mismatch_cost();
        LOG_DEBUG << "h(" << st.i << ", " << st.v << ") = ("
                  << max_seeds_to_end << "-" << matching_seeds << ") * "
    ...
    void before_every_alignment(const read_t *r) {
        ...
        marked_states = 0;
        r_ = r;
        seeds = gen_pivots_and_update(r, pivot_len, +1);
    ...
    void print_stats(std::ostream &out) const {
        ...
        out << " Total seed matches for all reads: " << seed_matches_
            << "(" << 1.0*seed_matches_/reads << " per read)"                    << std::endl;
  ...
  private:
    ...
    bool update_path_backwards(int p, int i, node_t v, int dval, int max_shifts) {
        ...
        //assert(__builtin_popcount(H[v]) <= seeds);
...
//                match_pivot_and_update(r, start, pivot_len, i+1, it->to, dval);
    ...
    int gen_pivots_and_update(const read_t *r, int pivot_len, int dval) {
        ...
        for (int i=r->len-pivot_len; i>=0; i-=pivot_len) {
            ...
            match_pivot_and_update(r, seeds, i, pivot_len, i, 0, dval);
            seeds++;
        }
        return seeds;
    }
